...
contract Ticket {
    ...
    bytes32 public constant PERMIT_TYPEHASH = keccak256("Permit(address from,address to,uint256 tokenId,uint256 nonce,uint256 deadline)");
    ...
    /* 
    NOTE how to solve:     
    Create the Permit Message: Construct a message that includes the relevant details (addresses, tokenId, nonce, deadline). This message should follow the EIP-712 standard for typed structured data hashing and signing.

    Sign the Message: The owner of the token (the from address) signs this message. The signing can be done in various environments (like in a web3 provider, a hardware wallet, etc.).

    Call transferWithPermit: The signed message is then used to call the transferWithPermit function. The v, r, and s parameters are derived from this signature, and the other parameters are the same as those used in the signed message.
     
    ==> Since _transfer function sets the owner of the tokenId to the to address, this effectively results in minting a new ticket to the to address. 
    */
    ...
    function _getDomainSeparator() public view returns (bytes32) {
